...
function prepareNextInvocation() {
  if (invocations.length > 0 && currentInvocation !== invocations[0]) {
    ...
    currentInvocation.timerID = runOnDate(currentInvocation.fireDate, function() {
      ...
      if(require('./utils/rruleUtil').doesRuleRecurr(cinv.recurrenceRule)) {
...
function scheduleNextRecurrence(rule, job, prevDate, endDate) {
  ...
  let date = null;
  if(require('./utils/rruleUtil').hasDependency()) {
    if (require('./utils/rruleUtil').isRRuleObject(rule)) {
      date = require('./utils/rruleUtil').nextInvocationDate(rule, prevDate);
    }
  }
