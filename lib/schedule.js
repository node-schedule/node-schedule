...
function Job(name, job, callback) {
  ...
  // Set scope vars
  const jobName = name && typeof name === 'string' ? name : '<Anonymous Job ' + (++anonJobCounter) + '>';
  ...
  this.stopTrackingInvocation = function(invocation) {
    const invIdx = pendingInvocations.indexOf(invocation);
    if (invIdx > -1) {
  ...
  this.cancel = function(reschedule) {
    ...
    let inv, newInv;
    const newInvs = [];
    for (let j = 0; j < pendingInvocations.length; j++) {
  ...
  this.cancelNext = function(reschedule) {
    ...
    let newInv;
    const nextInv = pendingInvocations.shift();
  ...
  this.reschedule = function(spec) {
    let inv;
    const cInvs = pendingInvocations.slice();

    for (let j = 0; j < cInvs.length; j++) {
      inv = cInvs[j];
...
Job.prototype.schedule = function(spec) {
  ...
  try {
    ...
    if ((type === 'string') || (type === 'number')) {
      // try to parse rrulestring
      try{
        spec = rrulestr(spec);
      } catch (rruleErr) {
        // if invalid rrule do nothing
      }
    ...
    } else if ( spec instanceof RRule || spec instanceof RRuleSet) {
...
function prepareNextInvocation() {
  if (invocations.length > 0 && currentInvocation !== invocations[0]) {
    ...
    currentInvocation.timerID = runOnDate(currentInvocation.fireDate, function() {
      ...
      } else if((cinv.recurrenceRule instanceof RRule || cinv.recurrenceRule instanceof RRuleSet)) {
        if(cinv.recurrenceRule.after(new Date())) {
          const inv = scheduleNextRecurrence(cinv.recurrenceRule, cinv.job, cinv.fireDate);
          if (inv !== null) {
            inv.job.trackInvocation(inv);
          }
        }
...
function scheduleNextRecurrence(rule, job, prevDate, endDate) {
  ...
  if (rule instanceof RecurrenceRule) {
    date = rule.nextInvocationDate(prevDate);
  } else if (rule instanceof RRule || rule instanceof RRuleSet) {
    date = rule.after(prevDate.toDate());
  } else {
    date = rule.next();
  }
