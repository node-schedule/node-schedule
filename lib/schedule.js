...
Job.prototype.schedule = function(spec) {
  ...
  if (typeof spec === 'object' && spec.rule) {
    ...
    if (start != null) {
      if (!(start instanceof CronDate)) {
        start = new CronDate(start, timezone);
      }
    ...
    if (end != null && !(end instanceof Date) && !(end instanceof CronDate)
      && !isValidDate(end = new CronDate(end, timezone))) {
      end = null;
    }
  }
  ...
  try {
    ...
    var res = cronParser.parseExpression(spec, { currentDate: start||undefined,  tz: timezone });
    ...
    if (type === 'number') {
      spec = new CronDate(spec, timezone||null);
    }


    if ((spec instanceof Date || spec instanceof CronDate) && (isValidDate(spec))) {
...
/* Invocation object */
function Invocation(job, fireDate, recurrenceRule, endDate, timezone) {
  this.job = job;
...
*/
function RecurrenceRule(year, month, date, dayOfWeek, hour, minute, second, timezone) {
  this.recurs = true;
  ...
  this.timezone = (timezone == null) ? null : timezone;
}

RecurrenceRule.prototype.nextInvocationDate = function(base) {
  base = (base instanceof Date || base instanceof CronDate) ? base : (new CronDate(undefined, this.timezone));
  if (!this.recurs) {
    return null;
  }

  var now = new CronDate(undefined, this.timezone);
  ...
  var next = new CronDate(base.getTime(), this.timezone);
  next.addSecond();
...
function scheduleNextRecurrence(rule, job, prevDate, endDate) {
  ...
  prevDate = (prevDate instanceof Date || prevDate instanceof CronDate) ? prevDate : (new CronDate(undefined, rule.timezone || null));
  ...
  if ((endDate instanceof Date || endDate instanceof CronDate) && date.getTime() > endDate.getTime()) {
    return null;
  }
