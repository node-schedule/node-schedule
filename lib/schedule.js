...
function scheduleInvocation(invocation) {
  if(invocation.fireDate.getTime() >= Date.now()) {
    invocations.push(invocation);
    invocations.sort(sorter);
    invocation.job.emit('scheduled', invocation.fireDate);
  }
  else if (invocation.recurrenceRule.recurs || invocation.recurrenceRule._endDate === null) {
    var inv = scheduleNextRecurrence(invocation.recurrenceRule, invocation.job, invocation.fireDate);
    if (inv !== null) {
