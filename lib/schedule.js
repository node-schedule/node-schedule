...
Job.prototype.nextDates = function(count = 1) {

  var pendingInvocations = this.pendingInvocations();
  ...
  var dates = [];
  var rule = pendingInvocations[0].recurrenceRule;
  var endDate = pendingInvocations[0].endDate;
  ...
  for (var i = 0; i < count; i++) {
    var next = getNextRecurrence(rule, prevDate, endDate);
    if (next) {
      prevDate = new CronDate(next.getTime());
      dates.push(next);
    } else {
      break;
    }
  ...
  // Bring Cron rule back to current time
  if (!(rule instanceof RecurrenceRule)) {
    rule.reset();
    while(rule.next().getTime() < Date.now()) { }
  }
...
function scheduleNextRecurrence(rule, job, prevDate, endDate) {
  ...
  var inv = new Invocation(job, next, rule, endDate);
  scheduleInvocation(inv);
...
/** Recurrence helper */
function getNextRecurrence(rule, prevDate, endDate) {
