...
Job.prototype.schedule = function(spec) {
  ...
  if (typeof spec === 'object' && spec.rule) {
    ...
    if (!(start instanceof Date) && !isValidDate(start = new Date(start))) {
      start = null;
    }

    if (!(end instanceof Date) && !isValidDate(end = new Date(end))) {
      end = null;
    }
  }
  ...
  if (invocations.length > 0 && currentInvocation !== invocations[0]) {
    ...
    currentInvocation.timerID = runOnDate(currentInvocation.fireDate, function() {
      ...
      if (cinv.recurrenceRule.recurs || cinv.recurrenceRule._endDate === null) {
        inv = scheduleNextRecurrence(cinv.recurrenceRule, cinv.job, cinv.fireDate, cinv.endDate);
        if (inv !== null) {
      ...
      job.invoke();
      job.emit('run', inv !== null ? inv.fireDate : null, cinv.endDate);
    });
  }
}
